...
void cfw_app_scene_protocols_freqs_static_var_item_list_callback(void* context, uint32_t index) {
    CFWApp* app = context;
    view_dispatcher_send_custom_event(app->view_dispatcher, index);
}

static void cfw_app_scene_protocols_freqs_static_frequency_changed(VariableItem* item) {
...
void cfw_app_scene_protocols_freqs_static_on_enter(void* context) {
    CFWApp* app = context;
    ...
    item = variable_item_list_add(
        ...
        cfw_app_scene_protocols_freqs_static_frequency_changed,
        app);
    ...
    variable_item_list_set_enter_callback(
        var_item_list, cfw_app_scene_protocols_freqs_static_var_item_list_callback, app);

    variable_item_list_set_selected_item(
        var_item_list,
        scene_manager_get_scene_state(app->scene_manager, CFWAppSceneProtocolsFreqsStatic));

    view_dispatcher_switch_to_view(app->view_dispatcher, CFWAppViewVarItemList);
}

bool cfw_app_scene_protocols_freqs_static_on_event(void* context, SceneManagerEvent event) {
    CFWApp* app = context;
    bool consumed = false;

    if(event.type == SceneManagerEventTypeCustom) {
        scene_manager_set_scene_state(
            app->scene_manager, CFWAppSceneProtocolsFreqsStatic, event.event);
        consumed = true;
        switch(event.event) {
        ...
        case VarItemListIndexAddStaticFreq:
            scene_manager_set_scene_state(app->scene_manager, CFWAppSceneProtocolsFreqsAdd, false);
            scene_manager_next_scene(app->scene_manager, CFWAppSceneProtocolsFreqsAdd);
            break;
        default:
            break;
        }
...
void cfw_app_scene_protocols_freqs_static_on_exit(void* context) {
    CFWApp* app = context;
