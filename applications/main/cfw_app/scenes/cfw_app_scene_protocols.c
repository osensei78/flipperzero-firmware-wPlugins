...
static void cfw_app_scene_protocols_file_naming_prefix_changed(VariableItem* item) {
    CFWApp* app = variable_item_get_context(item);
    bool value = variable_item_get_current_value_index(item);
    variable_item_set_current_value_text(item, value ? "After" : "Before");
    cfw_settings.file_naming_prefix_after = value;
...
void cfw_app_scene_protocols_on_enter(void* context) {
    ...
    item = variable_item_list_add(
        ...
        2,
        cfw_app_scene_protocols_subghz_bypass_changed,
        app);
    ...
    item = variable_item_list_add(
        ...
        2,
        cfw_app_scene_protocols_subghz_extend_changed,
        app);
    ...
    item = variable_item_list_add(
        ...
        "File Naming Prefix",
        2,
        cfw_app_scene_protocols_file_naming_prefix_changed,
        app);
    variable_item_set_current_value_index(item, cfw_settings.file_naming_prefix_after);
    variable_item_set_current_value_text(
        item, cfw_settings.file_naming_prefix_after ? "After" : "Before");
...
bool cfw_app_scene_protocols_on_event(void* context, SceneManagerEvent event) {
    ...
    if(event.type == SceneManagerEventTypeCustom) {
        ...
        case VarItemListIndexSubghzFreqs:
            scene_manager_set_scene_state(app->scene_manager, CFWAppSceneProtocolsFreqs, 0);
            scene_manager_next_scene(app->scene_manager, CFWAppSceneProtocolsFreqs);
            break;
        ...
        case VarItemListIndexGpioPins:
            scene_manager_set_scene_state(app->scene_manager, CFWAppSceneProtocolsGpio, 0);
            scene_manager_next_scene(app->scene_manager, CFWAppSceneProtocolsGpio);
            break;
        default:
            break;
        }
