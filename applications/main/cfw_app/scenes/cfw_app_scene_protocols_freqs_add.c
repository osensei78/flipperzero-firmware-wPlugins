...
static void
    cfw_app_scene_protocols_freqs_add_number_input_callback(void* context, int32_t number) {
    CFWApp* app = context;
    ...
    bool is_hopper =
        scene_manager_get_scene_state(app->scene_manager, CFWAppSceneProtocolsFreqsAdd);
    if(is_hopper) {
...
void cfw_app_scene_protocols_freqs_add_on_enter(void* context) {
    CFWApp* app = context;
    ...
    number_input_set_result_callback(
        number_input,
        cfw_app_scene_protocols_freqs_add_number_input_callback,
        app,
        0,
        100000,
        999999);

    view_dispatcher_switch_to_view(app->view_dispatcher, CFWAppViewNumberInput);
}
...
bool cfw_app_scene_protocols_freqs_add_on_event(void* context, SceneManagerEvent event) {
    CFWApp* app = context;
    bool consumed = false;
...
void cfw_app_scene_protocols_freqs_add_on_exit(void* context) {
    CFWApp* app = context;
