...
void cfw_app_scene_protocols_gpio_var_item_list_callback(void* context, uint32_t index) {
    CFWApp* app = context;
    view_dispatcher_send_custom_event(app->view_dispatcher, index);
}

static void cfw_app_scene_protocols_gpio_cc1101_handle_changed(VariableItem* item) {
...
static void cfw_app_scene_protocols_gpio_nrf24_handle_changed(VariableItem* item) {
    CFWApp* app = variable_item_get_context(item);
...
static void cfw_app_scene_protocols_gpio_esp32_channel_changed(VariableItem* item) {
    CFWApp* app = variable_item_get_context(item);
...
static void cfw_app_scene_protocols_gpio_nmea_channel_changed(VariableItem* item) {
    CFWApp* app = variable_item_get_context(item);
...
void cfw_app_scene_protocols_gpio_on_enter(void* context) {
    CFWApp* app = context;
    VariableItemList* var_item_list = app->var_item_list;
    VariableItem* item;

    item = variable_item_list_add(
        var_item_list, "CC1101 SPI", 2, cfw_app_scene_protocols_gpio_cc1101_handle_changed, app);
    variable_item_set_current_value_index(item, cfw_settings.spi_cc1101_handle);
    variable_item_set_current_value_text(
        item, cfw_settings.spi_cc1101_handle == SpiDefault ? SPI_DEFAULT : SPI_EXTRA);

    item = variable_item_list_add(
        var_item_list, "NRF24 SPI", 2, cfw_app_scene_protocols_gpio_nrf24_handle_changed, app);
    ...
    item = variable_item_list_add(
        ...
        "ESP32/8266 UART",
        2,
        cfw_app_scene_protocols_gpio_esp32_channel_changed,
        app);
    ...
    item = variable_item_list_add(
        var_item_list, "NMEA GPS UART", 2, cfw_app_scene_protocols_gpio_nmea_channel_changed, app);
    variable_item_set_current_value_index(item, cfw_settings.uart_nmea_channel);
    variable_item_set_current_value_text(
        item, cfw_settings.uart_nmea_channel == FuriHalSerialIdUsart ? UART_DEFAULT : UART_EXTRA);

    variable_item_list_set_enter_callback(
        var_item_list, cfw_app_scene_protocols_gpio_var_item_list_callback, app);

    variable_item_list_set_selected_item(
        var_item_list,
        scene_manager_get_scene_state(app->scene_manager, CFWAppSceneProtocolsGpio));

    view_dispatcher_switch_to_view(app->view_dispatcher, CFWAppViewVarItemList);
}

bool cfw_app_scene_protocols_gpio_on_event(void* context, SceneManagerEvent event) {
    CFWApp* app = context;
    bool consumed = false;

    if(event.type == SceneManagerEventTypeCustom) {
        scene_manager_set_scene_state(app->scene_manager, CFWAppSceneProtocolsGpio, event.event);
        consumed = true;
        switch(event.event) {
        default:
            break;
...
void cfw_app_scene_protocols_gpio_on_exit(void* context) {
    CFWApp* app = context;
