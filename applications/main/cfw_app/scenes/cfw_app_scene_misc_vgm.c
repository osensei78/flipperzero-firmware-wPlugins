...
static void cfw_app_scene_misc_vgm_foreground_changed(VariableItem* item) {
    ...
    cfw_settings.rpc_color_fg.rgb = vgm_colors[index].color;

    if(strcmp("Default", vgm_colors[index].name) == 0) {
        cfw_settings.rpc_color_fg.mode = ScreenColorModeDefault;
    } else if(strcmp("Rainbow", vgm_colors[index].name) == 0) {
        cfw_settings.rpc_color_fg.mode = ScreenColorModeRainbow;
    } else if(strcmp("RgbMod", vgm_colors[index].name) == 0) {
        cfw_settings.rpc_color_fg.mode = ScreenColorModeRgbBacklight;
    } else {
        cfw_settings.rpc_color_fg.mode = ScreenColorModeCustom;
    }
...
static void cfw_app_scene_misc_vgm_background_changed(VariableItem* item) {
    ...
    uint8_t index = variable_item_get_current_value_index(item);
    variable_item_set_current_value_text(item, vgm_colors[index].name);
    cfw_settings.rpc_color_bg.rgb = vgm_colors[index].color;

    if(strcmp("Default", vgm_colors[index].name) == 0) {
        cfw_settings.rpc_color_bg.mode = ScreenColorModeDefault;
    } else if(strcmp("Rainbow", vgm_colors[index].name) == 0) {
        cfw_settings.rpc_color_bg.mode = ScreenColorModeRainbow;
    } else if(strcmp("RgbMod", vgm_colors[index].name) == 0) {
        cfw_settings.rpc_color_bg.mode = ScreenColorModeRgbBacklight;
    } else {
        cfw_settings.rpc_color_bg.mode = ScreenColorModeCustom;
    }
...
void cfw_app_scene_misc_vgm_on_enter(void* context) {
    ...
    item = variable_item_list_add(
        ...
        app);
    ScreenFrameColor color = cfw_settings.rpc_color_fg;
    bool found = true;
    ...
    item = variable_item_list_add(
        ...
        app);
    color = cfw_settings.rpc_color_bg;
    found = true;
    if(color.mode == ScreenColorModeRainbow) {
        value_index = 1;
    } else if(color.mode == ScreenColorModeRgbBacklight) {
