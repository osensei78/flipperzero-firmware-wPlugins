...
static void cfw_app_scene_misc_screen_rgb_backlight_changed(VariableItem* item) {
    CFWApp* app = variable_item_get_context(item);
...
void cfw_app_scene_misc_screen_on_enter(void* context) {
    ...
    item = variable_item_list_add(
        var_item_list, "RGB Backlight", 2, cfw_app_scene_misc_screen_rgb_backlight_changed, app);
    value_index = cfw_settings.rgb_backlight;
    variable_item_set_current_value_index(item, value_index);
    variable_item_set_current_value_text(item, value_index ? "ON" : "OFF");
...
bool cfw_app_scene_misc_screen_on_event(void* context, SceneManagerEvent event) {
    ...
    if(event.type == SceneManagerEventTypeCustom) {
        ...
        case VarItemListIndexRgbBacklight: {
            ...
            bool value = variable_item_get_current_value_index(item);
            if(value == cfw_settings.rgb_backlight) value = !value; // Invoked via click
            ...
            if(change) {
                cfw_settings.rgb_backlight = value;
                app->save_settings = true;
                app->save_backlight = true;
