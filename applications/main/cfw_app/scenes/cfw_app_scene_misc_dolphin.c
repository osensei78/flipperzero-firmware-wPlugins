...
void cfw_app_scene_misc_dolphin_var_item_list_callback(void* context, uint32_t index) {
    CFWApp* app = context;
    view_dispatcher_send_custom_event(app->view_dispatcher, index);
}

static void cfw_app_scene_misc_dolphin_dolphin_level_changed(VariableItem* item) {
...
static void cfw_app_scene_misc_dolphin_dolphin_xp_changed(VariableItem* item) {
    CFWApp* app = variable_item_get_context(item);
...
static void cfw_app_scene_misc_dolphin_dolphin_angry_changed(VariableItem* item) {
    CFWApp* app = variable_item_get_context(item);
...
static void cfw_app_scene_misc_dolphin_butthurt_timer_changed(VariableItem* item) {
    CFWApp* app = variable_item_get_context(item);
    uint8_t index = variable_item_get_current_value_index(item);
...
void cfw_app_scene_misc_dolphin_on_enter(void* context) {
    CFWApp* app = context;
    ...
    item = variable_item_list_add(
        ...
        cfw_app_scene_misc_dolphin_dolphin_level_changed,
        app);
    ...
    // show buttons and change values in callback
    item = variable_item_list_add(
        var_item_list, "Dolphin XP", 3, cfw_app_scene_misc_dolphin_dolphin_xp_changed, app);
    variable_item_set_current_value_index(
        item,
        app->dolphin_xp == 0              ? 0 :
        app->dolphin_xp == DOLPHIN_MAX_XP ? 2 :
    ...
    item = variable_item_list_add(
        ...
        "Dolphin Angry",
        BUTTHURT_MAX + 1,
        cfw_app_scene_misc_dolphin_dolphin_angry_changed,
        app);
    ...
    item = variable_item_list_add(
        ...
        cfw_app_scene_misc_dolphin_butthurt_timer_changed,
        app);
    value_index = value_index_uint32(
        cfw_settings.butthurt_timer, butthurt_timer_values, COUNT_OF(butthurt_timer_values));
    ...
    variable_item_list_set_enter_callback(
        var_item_list, cfw_app_scene_misc_dolphin_var_item_list_callback, app);

    variable_item_list_set_selected_item(
        var_item_list, scene_manager_get_scene_state(app->scene_manager, CFWAppSceneMiscDolphin));

    view_dispatcher_switch_to_view(app->view_dispatcher, CFWAppViewVarItemList);
}

bool cfw_app_scene_misc_dolphin_on_event(void* context, SceneManagerEvent event) {
    CFWApp* app = context;
    bool consumed = false;

    if(event.type == SceneManagerEventTypeCustom) {
        scene_manager_set_scene_state(app->scene_manager, CFWAppSceneMiscDolphin, event.event);
        consumed = true;
        switch(event.event) {
        case VarItemListIndexDolphinXp:
            scene_manager_next_scene(app->scene_manager, CFWAppSceneMiscDolphinXp);
            break;
        default:
            break;
        }
...
void cfw_app_scene_misc_dolphin_on_exit(void* context) {
    CFWApp* app = context;
