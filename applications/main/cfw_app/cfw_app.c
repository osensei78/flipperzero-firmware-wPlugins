...
static void cfw_app_push_mainmenu_app_raw(CFWApp* app, FuriString* label, FuriString* exe) {
    ...
    if(furi_string_equal(label, "CFW")) {
        furi_string_set(label, "CFW");
...
void cfw_app_push_mainmenu_app(CFWApp* app, FuriString* exe) {
    ...
    if(furi_string_start_with(exe, "/")) {
        ...
        cfw_app_push_mainmenu_app_raw(app, label, exe);
    } else {
        bool found = false;
    ...
    if(file_stream_open(stream, MAINMENU_APPS_PATH, FSAM_READ, FSOM_OPEN_EXISTING) &&
       ...
       version <= 1) {
        while(stream_read_line(stream, line)) {
            ...
            cfw_app_push_mainmenu_app(app, line);
        }
    } else {
        for(size_t i = 0; i < FLIPPER_APPS_COUNT; i++) {
            furi_string_set(label, FLIPPER_APPS[i].name);
            furi_string_set(line, FLIPPER_APPS[i].name);
            cfw_app_push_mainmenu_app_raw(app, label, line);
        }
...
CFWApp* cfw_app_alloc() {
    ...
    view_dispatcher_add_view(
        app->view_dispatcher, CFWAppViewNumberInput, number_input_get_view(app->number_input));
        ...
        while(storage_dir_read(folder, &info, name, ASSET_PACKS_NAME_LEN)) {
            if(info.flags & FSF_DIRECTORY && name[0] != '.') {
                ...
                if(app->asset_pack_index != 0) {
                    ...
                    if(strcmp(copy, cfw_settings.asset_pack) == 0) app->asset_pack_index = idx + 1;
                }
            }
        }
    }
    ...
    if(furi_string_equal(app->version_tag, "rm-dev")) {
        ...
        furi_string_replace(app->version_tag, "rm", "RM");
...
void cfw_app_free(CFWApp* app) {
    ...
    view_dispatcher_remove_view(app->view_dispatcher, CFWAppViewNumberInput);
