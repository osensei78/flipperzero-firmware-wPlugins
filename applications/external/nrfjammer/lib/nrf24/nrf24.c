...
void nrf24_startConstCarrier(const FuriHalSpiBusHandle* handle, uint8_t level, uint8_t channel) {
    nrf24_deinit();
    nrf24_init();
...
void nrf24_stopConstCarrier(const FuriHalSpiBusHandle* handle) {
    uint8_t setup;
    nrf24_read_reg(handle, REG_RF_SETUP, &setup, 1);
    setup &= ~(NRF24_CONT_WAVE | NRF24_PLL_LOCK);
    nrf24_write_reg(handle, REG_RF_SETUP, setup);
