...
bool fs_init(const fs_init_params_t* p) {
    ...
    if(g.mode == FS_MODE_SENDER) {
        ...
        FURI_LOG_I(
            TAG,
            "fs_init: SENDER, file_path='%s', file_name='%s', file_size=%lu, mode=%d, tx_id=%d",
            g.s_file_path,
            g.s_file_name,
            g.s_file_size,
            g.mode,
    ...
    if(g.mode == FS_MODE_RECEIVER) {
        ...
        if(now - g.last_rx_ms > FS_RX_TIMEOUT_MS) {
            ...
            if(g.r_locked && g.r_is_finished == false) {
                if(g.r_blocks_received < g.r_blocks_needed) { // find next needed block window
                    ...
                    FURI_LOG_I(
                        TAG,
                        "fs_idle: sending REQUEST for blocks range (%lu, %lu), bytes (%lu, %lu), delay=%lums",
                        nblk_start,
                        nblk_end,
                        nbyte_start,
                        nbyte_end,
        ...
        if(g.r_locked && g.r_blocks_received == g.r_blocks_needed) {
            ...
            if(err != 0) {
                ...
                FURI_LOG_I(
                    TAG,
                    "fs_idle: Received file md5=%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X",
                    real_md5[0],
                    real_md5[1],
                    real_md5[2],
                    real_md5[3],
