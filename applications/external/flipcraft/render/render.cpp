...
static void initTrigLUT() {
    ...
    auto fsinf = [](float a) {
        return FixedPoint(fabsf(sinf(a)), 16, 14) * (sinf(a) > 0 ? 1.0f : -1.0f);
    };
    auto fcosf = [](float a) {
        return FixedPoint(fabsf(cosf(a)), 16, 14) * (cosf(a) > 0 ? 1.0f : -1.0f);
    };
    for(int i = 0; i < 16; i++) {
...
float Renderer::sinYaw() const {
    return kSinYaw[yawIndex & 0xF];
}
float Renderer::cosYaw() const {
...
void Renderer::renderOverlay(const World& w, int x, int y, int z, int breakPhase) {
    ...
    for(int i = 0; i < 6; i++) {
        ...
        if(blockIsTransparent(adj)) renderQuad(x, y, z, BlockQuads[i], texId, 0b1101);
    }
}
...
void Renderer::renderScene(const World& w) {
    ...
    for(int axis = 0; axis < 3; axis++) {
        ...
        if(axis == 0) {
            currentFace = QUAD_FULL_NEGX;
            previousFace = QUAD_FULL_POSX;
            currentTexIndex = 2;
            previousTexIndex = 2;
        } else if(axis == 1) {
            currentFace = QUAD_FULL_NEGZ;
            previousFace = QUAD_FULL_POSZ;
            currentTexIndex = 2;
            previousTexIndex = 2;
        } else {
            currentFace = QUAD_FULL_NEGY;
            previousFace = QUAD_FULL_POSY;
            currentTexIndex = 1;
            previousTexIndex = 0;
        }
        ...
        for(int a1 = a1lo; a1 <= a1hi; a1++)
            for(int a2 = a2lo; a2 <= a2hi; a2++) {
                uint8_t previous = BLOCK_AIR;
                bool previousFull = false, previousTransparent = false;
                        ...
                        if(pos[1] == 0 && currentTransparent)
                            ...
                            if(m.exists && m.texCount > 0)
                                renderQuad(
                                    ...
                                    QUAD_FULL_POSY,
                                    m.textures[0].id,
                                    m.textures[0].settings);
                        }
                        if(!currentFull && current != BLOCK_AIR) {
                            const MeshEntry& m = meshBlock(current);
                            if(m.exists)
                                for(int qi = 0; qi < m.quadCount; qi++) {
