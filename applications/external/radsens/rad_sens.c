    ...
    if(event->type == InputTypeShort || event->type == InputTypeRepeat) {
        ...
        case InputKeyOk:
            if(event->type == InputTypeShort) {
                with_view_model(
                    app->view, RadSensModel * model, { model->vibro_on = !model->vibro_on; }, true);
            }
            consumed = true;
            break;
        case InputKeyRight:
            if(event->type == InputTypeShort) {
                with_view_model(
                    app->view, RadSensModel * model, { model->show_history = true; }, true);
            }
            consumed = true;
            break;
        case InputKeyLeft:
            if(event->type == InputTypeShort) {
                with_view_model(
                    app->view, RadSensModel * model, { model->show_history = false; }, true);
            }
            consumed = true;
            break;
        case InputKeyUp:
            furi_thread_flags_set(
                furi_thread_get_id(app->worker_thread), WorkerEventSensitivityUp);
            consumed = true;
            break;
        case InputKeyDown:
            furi_thread_flags_set(
