...
void scene_main_draw_cb(Canvas* canvas, void* model) {
    ...
    canvas_draw_str_aligned(
        canvas, SCREEN_W / 2, TITLE_H - 4, AlignCenter, AlignBottom, "Polk Magnifi");
...
bool scene_main_input_cb(InputEvent* event, void* ctx) {
    ...
    if(event->type == InputTypePress &&
       (event->key == InputKeyLeft || event->key == InputKeyOk || event->key == InputKeyRight)) {
        ...
        with_view_model(
            ...
            {
                m->is_pressed = true;
                m->pressed_row = app->selected_row;
                m->pressed_cell = cell;
            },
            true);
        return false;
    ...
    if(event->type == InputTypeRelease) {
        with_view_model(app->main_view, MainViewModel * m, { m->is_pressed = false; }, true);
        return false;
    }
        ...
        if(app->selected_row > 0) {
            ...
            with_view_model(
                ...
                MainViewModel * m,
                {
                    ...
                    if(m->selected_row < m->scroll_offset) m->scroll_offset = m->selected_row;
                },
                true);
        }
        return true;
    ...
    if(event->key == InputKeyOk) {
        if(row->type == RowSubmenu) {
            ...
            view_dispatcher_send_custom_event(app->view_dispatcher, PolkEventOpenSubmenu);
        } else {
...
void scene_main_on_enter(void* ctx) {
    PolkRemoteApp* app = ctx;
    with_view_model(
        app->main_view, MainViewModel * m, { m->selected_row = app->selected_row; }, true);
    view_dispatcher_switch_to_view(app->view_dispatcher, SceneMain);
}

bool scene_main_on_event(void* ctx, SceneManagerEvent event) {
    PolkRemoteApp* app = ctx;
    if(event.type == SceneManagerEventTypeCustom && event.event == PolkEventOpenSubmenu) {
