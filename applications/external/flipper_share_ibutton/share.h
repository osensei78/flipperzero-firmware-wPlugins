...
// All transport-tunable constants live here (per-app), so this engine header
// stays byte-identical across the new Flipper Share apps and defines none of
// them itself.
#include "share_config.h"

// ===== Packet sizing =========================================================
// Wire format per packet: [version(1)][tx_id(1)][packet_type(1)][payload][crc16(2)]
#define FSH_HEADER_LENGTH   3u // version + tx_id + packet_type
#define FSH_CRC_LENGTH      2u // CRC-16 trailer
#define FSH_OVERHEAD_LENGTH (FSH_HEADER_LENGTH + FSH_CRC_LENGTH)

// Control packets (ANNOUNCE / REQUEST) must carry file_name + file_size + md5.
#define FSH_FILENAME_LENGTH     36u // max file name incl. NUL
#define FSH_CTRL_PAYLOAD_LENGTH (FSH_FILENAME_LENGTH + 4u + 16u) // name + size + md5
#define FSH_CTRL_PACKET_LENGTH  (FSH_CTRL_PAYLOAD_LENGTH + FSH_OVERHEAD_LENGTH)

// DATA packets: block_number(4) + file data. FSH_DATA_LENGTH is THE tunable knob
...
// single length byte of the 1-Wire link layer (asserted in ibutton_transport.c).
...
#define FSH_PAYLOAD_MAX                                                              \
    ((FSH_CTRL_PAYLOAD_LENGTH > FSH_DATA_PAYLOAD_LENGTH) ? FSH_CTRL_PAYLOAD_LENGTH : \
                                                           FSH_DATA_PAYLOAD_LENGTH)
#define FSH_PACKET_MAX (FSH_PAYLOAD_MAX + FSH_OVERHEAD_LENGTH)

// ===== Behaviour tunables ====================================================
// FSH_PAYLOAD_THROUGHPUT_BPS and FSH_STALL_MS are transport-tunable and live in
// share_config.h. The rest below are transport-independent engine constants.
#define FSH_ETA_WARMUP_MS 3000 // below this elapsed, ETA uses the constant fallback
#define FSH_ETA_MAX_SEC \
    (99u * 3600u + 59u * 60u + 59u) // clamp ETA display (avoid uint32 overflow / garbage)

#define FSH_HASH_CHUNK_SIZE  4096 // bytes per MD5 chunk (hash progress granularity)
#define FSH_WORKER_STOP_FLAG 0x1u // FuriThread flag: stop worker (also aborts in-progress hashing)

#define FSH_PARTS_COUNT 100u // For progress bar in GUI
#define FSH_PARTS_BYTES ((uint32_t)((FSH_PARTS_COUNT + 7u) / 8u))
...
// Payloads:
typedef struct {
    uint8_t file_name[FSH_FILENAME_LENGTH];
    uint32_t file_size;
    uint8_t hash_md5[16];
} FSH_pl_announce_t;
...
typedef enum {
    FSH_ST_IDLE = 0,
    FSH_ST_ANNOUNCING, // sender: broadcasting metadata, waiting for a receiver
    FSH_ST_CONNECTED, // sender: a receiver locked on -> stop announcing, serve requests
    FSH_ST_RECEIVING // receiver
} fsh_state_t;
...
typedef struct {
    ...
    char s_file_name[FSH_FILENAME_LENGTH];
    ...
    // Callback for reading a data block (FSH_DATA_LENGTH bytes)
    // MUST fill the output buffer (zero-pad the incomplete last block).
    // Returns the actual number of "valid" bytes in this block: FSH_DATA_LENGTH,
    // or the remainder for the last one. (The receiver trims it based on file_size)
    uint32_t (*cb_read_block)(
        uint32_t block_number,
        uint8_t out[FSH_DATA_LENGTH]); // Reader callback from real storage
    ...
    char r_file_name[FSH_FILENAME_LENGTH];
    ...
    uint32_t r_finfsh_ms; // set when all blocks received (finalization) — reception end time
    ...
    void (*cb_write_block)(
        uint32_t block_number,
        const uint8_t in[FSH_DATA_LENGTH],
        uint32_t valid_len);
} fsh_ctx_t;
...
typedef struct {
    fsh_mode_t mode; // SENDER / RECEIVER
    uint8_t
    ...
    char s_file_path[256];
    uint32_t (*s_read_block)(uint32_t block_number, uint8_t out[FSH_DATA_LENGTH]);
...
// API functions:
bool fsh_init_from_external_transmit();
bool fsh_init_from_external_receive();
bool fsh_init(const fsh_init_params_t* p);
void fsh_deinit(void);
void fsh_idle(void); // to be called periodically from main loop (50ms?)

// Shared-state lock (protects `g`, g_map and fsh_parts across the worker,
// NFC RX-callback and GUI threads). Create it in the scene on_enter BEFORE
// starting any worker/radio thread; fsh_deinit() frees it.
void fsh_lock_ensure(void); // idempotent, alloc mutex if missing
void fsh_lock(void); // blocking acquire (no-op if not created)
void fsh_unlock(void); // release (no-op if not created)
bool fsh_try_lock_ms(uint32_t timeout_ms); // timed acquire, true if acquired (for GUI callbacks)
...
// void fsh_send_data(uint32_t block_number);
void fsh_send_data(void);

// ===== Engine <-> transport contract =========================================
// The engine is transport-agnostic and talks to the per-app transport layer
...
//       per-app transport (here ibutton_transport.c).
...
void fsh_parts_reset(void);
void fsh_parts_on_block_set(uint32_t block_index);
int fsh_parts_get(uint32_t part_index);
void fsh_parts_levels_copy(uint8_t* dst); // dst[FSH_PARTS_COUNT], per-part fill level 0..255
