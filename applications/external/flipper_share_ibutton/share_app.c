#include "share_app.h"
#include "share_config.h" // FSH_TRANSPORT_NAME for UI strings
...
// Set the callback after creating the file_browser (in share_alloc):
...
bool share_custom_event_callback(void* context, uint32_t event) {
    furi_assert(context);
    ShareApp* app = context;
    return scene_manager_handle_custom_event(app->scene_manager, event);
}

static bool share_back_event_callback(void* context) {
    furi_assert(context);
    ShareApp* app = context;
...
static void submenu_callback(void* context, uint32_t index) {
    ...
    if(index == 0) { // Send - open file browser
        scene_manager_next_scene(app->scene_manager, ShareSceneFileBrowser);
    } else if(index == 1) { // Receive
        scene_manager_next_scene(app->scene_manager, ShareSceneReceive);
        ...
        widget_add_text_box_element(
            app->widget_about, 0, 0, 128, 14, AlignCenter, AlignBottom, FSH_ABOUT_BLANK_INV, false);
        widget_add_text_box_element(
            app->widget_about, 0, 2, 128, 14, AlignCenter, AlignBottom, FSH_ABOUT_NAME, false);
        ...
        furi_string_cat_printf(
            about_text, "%s\n\n", "File transfer over the iButton pad (1-Wire). Touch pads.");
        furi_string_cat_printf(
            about_text,
        ...
        view_dispatcher_switch_to_view(app->view_dispatcher, ShareViewIdAbout);
    }
}

// Return to main menu when pressing Back on About
static uint32_t share_about_previous(void* context) {
    UNUSED(context);
    return ShareViewIdMenu;
}
...
/*
static bool read_file_info(ShareApp* app) {
...
// static bool read_file_content(ShareApp* app, char* buffer, size_t buffer_size) {
//     Storage* storage = furi_record_open(RECORD_STORAGE);
...
static ShareApp* share_alloc() {
    ShareApp* app = malloc(sizeof(ShareApp));
    ...
    submenu_add_item(app->submenu, "Send via " FSH_TRANSPORT_NAME, 0, submenu_callback, app);
    submenu_add_item(app->submenu, "Receive via " FSH_TRANSPORT_NAME, 1, submenu_callback, app);
    submenu_add_item(app->submenu, "About", 2, submenu_callback, app);
    view_dispatcher_add_view(
        app->view_dispatcher, ShareViewIdMenu, submenu_get_view(app->submenu));
    ...
    view_dispatcher_add_view(
        app->view_dispatcher, ShareViewIdAbout, widget_get_view(app->widget_about));
    // Ensure Back from About returns to Menu
    view_set_previous_callback(widget_get_view(app->widget_about), share_about_previous);

    return app;
}

static void share_free(ShareApp* app) {
    furi_assert(app);

    view_dispatcher_remove_view(app->view_dispatcher, ShareViewIdReceive);
    view_dispatcher_remove_view(app->view_dispatcher, ShareViewIdShowFile);
    view_dispatcher_remove_view(app->view_dispatcher, ShareViewIdFileBrowser);
    view_dispatcher_remove_view(app->view_dispatcher, ShareViewIdMenu);
    // Also remove the About view to avoid leaving a dangling view pointer
    view_dispatcher_remove_view(app->view_dispatcher, ShareViewIdAbout);
...
int32_t share_app(void* p) {
    UNUSED(p);
    ShareApp* app = share_alloc();

    view_dispatcher_attach_to_gui(app->view_dispatcher, app->gui, ViewDispatcherTypeFullscreen);

    scene_manager_next_scene(app->scene_manager, ShareSceneMenu);
