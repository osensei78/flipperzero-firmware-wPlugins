...
#define FSH_RECEIVER_DIRECTORY "inbox"

// ===== Constants of protocol / timings =====
// The wire-format version is transport-independent and stays here. All the
// timing tunables (FSH_ANNOUNCE_INTERVAL_MS, FSH_RX_TIMEOUT_MS, FSH_TX_TIMEOUT_MS,
...
fsh_ctx_t g; //static, extern in .h

// ===== Shared-state lock =====
// Protects `g`, g_map and fsh_parts against concurrent access from the worker
// thread (fsh_idle), the NFC RX-callback thread (fsh_handle_*) and the GUI
// timer/draw callbacks. Kept as a file-scope static (NOT a field of `g`, so
// memset(&g,0) in fsh_init does not clobber it).
static FuriMutex* g_lock = NULL;

void fsh_lock_ensure(void) {
    if(!g_lock) {
        g_lock = furi_mutex_alloc(FuriMutexTypeNormal);
    }
}

static void fsh_lock_free(void) {
    if(g_lock) {
...
void fsh_unlock(void) {
    if(g_lock) furi_mutex_release(g_lock);
}

bool fsh_try_lock_ms(uint32_t timeout_ms) {
...
// fsh_md5_calc_file_progress (sender init and receiver finalization). File-scope
...
bool fsh_hash_progress_get(uint32_t* done, uint32_t* total) {
    if(!fsh_try_lock_ms(10)) return false;
    *done = g_hash_done;
    *total = g_hash_total;
    fsh_unlock();
    return true;
...
//  - publishes progress for the GUI after every FSH_HASH_CHUNK_SIZE bytes;
...
static bool fsh_md5_calc_file_progress(
    File* file,
    const char* path,
    ...
    uint8_t* data = malloc(FSH_HASH_CHUNK_SIZE);
    Md5Context md5_ctx;
    ...
    while(true) {
        ...
        size_t read_size = storage_file_read(file, data, FSH_HASH_CHUNK_SIZE);
...
void fsh_notify_success(void) {
    NotificationApp* notification = furi_record_open(RECORD_NOTIFICATION);
    notification_message(notification, &sequence_success);
    furi_record_close(RECORD_NOTIFICATION);
}

void fsh_notify_error(void) {
    NotificationApp* notification = furi_record_open(RECORD_NOTIFICATION);
    notification_message(notification, &sequence_error);
    furi_record_close(RECORD_NOTIFICATION);
}

void fsh_notify_vibro(void) {
    NotificationApp* notification = furi_record_open(RECORD_NOTIFICATION);
    notification_message(notification, &sequence_single_vibro);
    furi_record_close(RECORD_NOTIFICATION);
}

void fsh_notify_led_red(void) {
    NotificationApp* notification = furi_record_open(RECORD_NOTIFICATION);
    notification_message(notification, &sequence_blink_red_10);
    furi_record_close(RECORD_NOTIFICATION);
}

void fsh_notify_led_green(void) {
    NotificationApp* notification = furi_record_open(RECORD_NOTIFICATION);
    notification_message(notification, &sequence_blink_green_10);
    furi_record_close(RECORD_NOTIFICATION);
}

void fsh_notify_led_blue(void) {
    NotificationApp* notification = furi_record_open(RECORD_NOTIFICATION);
    notification_message(notification, &sequence_blink_blue_10);
    furi_record_close(RECORD_NOTIFICATION);
}

void fsh_notify_led_cyan(void) {
    NotificationApp* notification = furi_record_open(RECORD_NOTIFICATION);
...
// which is the exact inverse of part(i)=floor(i*N/B) used in fsh_parts_on_block_set.
...
bool fsh_parts_init(uint32_t block_count) {
    memset(&fsh_parts, 0, sizeof(fsh_parts));
...
uint32_t fsh_parts_count(void) {
    return fsh_parts.N;
}

uint32_t fsh_parts_block_count(void) {
    return fsh_parts.B;
}
...
void fsh_fmt_duration(uint32_t secs, char* buf, size_t n) {
...
static inline size_t fsh_payload_len_for_type(uint8_t pkt_type) {
    return (pkt_type == FSH_PKT_DATA) ? FSH_DATA_PAYLOAD_LENGTH : FSH_CTRL_PAYLOAD_LENGTH;
}

static inline size_t fsh_packet_len_for_type(uint8_t pkt_type) {
    return FSH_HEADER_LENGTH + fsh_payload_len_for_type(pkt_type) + FSH_CRC_LENGTH;
}
...
static size_t fsh_packet_pack(
    ...
    out[1] = tx_id;
    out[2] = pkt_type;
    memcpy(out + FSH_HEADER_LENGTH, payload_src, pl);
    uint16_t crc = fsh_crc16(out, FSH_HEADER_LENGTH + pl);
    out[FSH_HEADER_LENGTH + pl] = (uint8_t)(crc & 0xFF);
    out[FSH_HEADER_LENGTH + pl + 1] = (uint8_t)(crc >> 8);
    return FSH_HEADER_LENGTH + pl + FSH_CRC_LENGTH;
}
...
static void fsh_pl_announce_pack(uint8_t* payload_out, const FSH_pl_announce_t* pl_src) {
    memcpy(payload_out, pl_src->file_name, FSH_FILENAME_LENGTH);
    memcpy(payload_out + FSH_FILENAME_LENGTH, &pl_src->file_size, sizeof(uint32_t)); // LE
    memcpy(payload_out + FSH_FILENAME_LENGTH + 4, pl_src->hash_md5, 16);
}

static void fsh_pl_announce_unpack(const uint8_t* payload, FSH_pl_announce_t* pl_out) {
    memcpy(pl_out->file_name, payload, FSH_FILENAME_LENGTH);
    memcpy(&pl_out->file_size, payload + FSH_FILENAME_LENGTH, sizeof(uint32_t));
    memcpy(pl_out->hash_md5, payload + FSH_FILENAME_LENGTH + 4, 16);
}

// REQUEST (control payload; unused bytes zero-padded)
static void fsh_pl_request_pack(uint8_t* payload_out, const FSH_pl_request_t* pl_src) {
    memset(payload_out, 0, FSH_CTRL_PAYLOAD_LENGTH);
    memcpy(payload_out + 0, &pl_src->range_start, sizeof(uint32_t));
    memcpy(payload_out + 4, &pl_src->range_end, sizeof(uint32_t));
}

static void fsh_pl_request_unpack(const uint8_t* payload, FSH_pl_request_t* pl_out) {
    memcpy(&pl_out->range_start, payload + 0, sizeof(uint32_t));
    memcpy(&pl_out->range_end, payload + 4, sizeof(uint32_t));
}

// DATA (data payload = block_number + FSH_DATA_LENGTH bytes)
static void fsh_pl_data_pack(uint8_t* payload_out, const FSH_pl_data_t* pl_src) {
    memcpy(payload_out + 0, &pl_src->block_number, sizeof(uint32_t));
    memcpy(payload_out + 4, pl_src->data, FSH_DATA_LENGTH);
}

static void fsh_pl_data_unpack(const uint8_t* payload, FSH_pl_data_t* pl_out) {
    memcpy(&pl_out->block_number, payload + 0, sizeof(uint32_t));
    memcpy(pl_out->data, payload + 4, FSH_DATA_LENGTH);
}

// ===== API Implementation =====

void fsh_ensure_inbox_dir(void) {
...
static void fsh_close_session_file(void) {
    if(g.file) {
...
uint8_t fsh_file_create_truncate(uint32_t file_size) {
    ...
    fsh_close_session_file();

    g.storage = furi_record_open(RECORD_STORAGE);
    g.file = storage_file_alloc(g.storage);
    if(!g.file) {
        FURI_LOG_E(TAG, "fsh_file_create_truncate: Failed to allocate file handle");
        fsh_close_session_file();
        return 2;
    }
    if(!storage_file_open(g.file, g.r_file_path, FSAM_WRITE, FSOM_CREATE_ALWAYS)) {
        FURI_LOG_E(TAG, "fsh_file_create_truncate: Failed to open file '%s'", g.r_file_path);
        fsh_close_session_file();
        return 3;
    }
    ...
    if(!storage_file_seek(g.file, file_size, true)) {
        FURI_LOG_E(TAG, "fsh_file_create_truncate: Failed to seek to offset %lu", file_size);
        fsh_close_session_file();
        return 4;
    }
...
// Called from fsh_handle_data (RX thread) while holding g_lock. Writes into the
// persistent session handle (opened once in fsh_file_create_truncate). FatFS
// coalesces sub-sector writes in the FIL's 512-byte buffer, so physical SD
// writes happen ~once per 512 bytes, not per block.
void my_write_block(uint32_t block_number, const uint8_t in[FSH_DATA_LENGTH], uint32_t valid_len) {
    if(!g.file) {
        FURI_LOG_E(TAG, "my_write_block: no open file handle");
        return;
    }
    uint32_t offset = block_number * FSH_DATA_LENGTH;
...
// fsh_init). Called from fsh_send_data on the worker thread under g_lock.
uint32_t my_read_block(uint32_t block_number, uint8_t out[FSH_DATA_LENGTH]) {
    memset(out, 0, FSH_DATA_LENGTH);
    if(!g.file) {
    ...
    return bytes; // number of valid bytes read, can be < FSH_DATA_LENGTH for last block
}

bool fsh_init_from_external_transmit(const char* file_path) {
    // FURI_LOG_I(TAG, "fsh_init_from_external_transmit: file_path='%s'", file_path);

    fsh_init_params_t ps;
    memset(&ps, 0, sizeof(ps));
    ps.mode = FSH_MODE_SENDER;
    ps.tx_id = (uint8_t)(furi_get_tick() & 0xFFu);
    ps.send_bytes = fsh_transport_send;
...
bool fsh_init(const fsh_init_params_t* p) {
    ...
    if(p->mode == FSH_MODE_SENDER) {
        ...
        // BEFORE touching `g`. fsh_md5_calc_file_progress() opens the file by path itself.
        ...
        FS_Error err = 0;
        bool md5_ok = fsh_md5_calc_file_progress(file, p->s_file_path, md5, &err);
        ...
        if(!md5_ok && (furi_thread_flags_get() & FSH_WORKER_STOP_FLAG)) {
            FURI_LOG_I(TAG, "fsh_init: hashing cancelled");
            return false;
        }
        ...
        strncpy(g.s_file_name, basename, FSH_FILENAME_LENGTH - 1);
        g.s_file_name[FSH_FILENAME_LENGTH - 1] = '\0';
        ...
        fsh_lock();
        memset(&g, 0, sizeof(g));
        g.mode = FSH_MODE_RECEIVER;
        g.state = FSH_ST_RECEIVING;
        g.cb_send_bytes = p->send_bytes;
        g.cb_now_ms = p->now_ms;
        g.last_tick_ms = g.last_announce_ms = g.last_rx_ms = p->now_ms();
        ...
        FURI_LOG_I(TAG, "fsh_init: RECEIVER");
    }
    return true;
}
...
void fsh_deinit(void) {
    FURI_LOG_I(TAG, "fsh_deinit");
    ...
    FURI_LOG_I(TAG, "fsh_deinit: done");
}
...
void fsh_send_announce(void) {
    ...
    FSH_pl_announce_t pl = {0};
    memcpy(pl.file_name, g.s_file_name, FSH_FILENAME_LENGTH);
    pl.file_size = g.s_file_size;
    memcpy(pl.hash_md5, g.s_md5, 16);

    uint8_t payload[FSH_PAYLOAD_MAX];
    fsh_pl_announce_pack(payload, &pl);

    uint8_t pkt[FSH_PACKET_MAX];
    size_t n = fsh_packet_pack(pkt, FSH_VERSION, g.tx_id, FSH_PKT_ANNOUNCE, payload);
...
void fsh_send_request(uint32_t range_start, uint32_t range_end) {
    ...
    uint8_t pkt[FSH_PACKET_MAX];
    size_t n = fsh_packet_pack(pkt, FSH_VERSION, g.tx_id, FSH_PKT_REQUEST, payload);
    g.cb_send_bytes(pkt, n);
}
...
void fsh_idle(void) {
    ...
    if(g.mode == FSH_MODE_RECEIVER) {
        ...
        // fsh_handle_announce/fsh_handle_data early-return, so nothing truncates or
...
static void fsh_handle_announce(uint8_t tx_id, const FSH_pl_announce_t* ann) {
    ...
    if(g.r_locked == false) {
        ...
        memcpy(g.r_file_name, name, FSH_FILENAME_LENGTH);
        ...
        // FSH_RX_TIMEOUT_MS: seed last_rx_ms in the past so the next fsh_idle tick
        // fires the REQUEST right away (faster session start).
        g.last_rx_ms = (g.cb_now_ms ? g.cb_now_ms() : 0) - (uint32_t)FSH_RX_TIMEOUT_MS - 1u;
...
static void fsh_handle_request(uint8_t tx_id, const FSH_pl_request_t* rq) {
    ...
    g.state = FSH_ST_CONNECTED; // a receiver is talking to us -> stop announcing
    notify_cyan = true;
    fsh_unlock();
...
static void fsh_handle_data(uint8_t tx_id, const FSH_pl_data_t* d) {
    ...
    // makes fsh_idle/GUI wait briefly and guarantees no interleave with the
    // finalization stage.
    fsh_lock();
    ...
    fsh_map_set(d->block_number, 1); // mark block as received
    fsh_parts_on_block_set(d->block_number); // handle parts
    g.r_blocks_received++;
