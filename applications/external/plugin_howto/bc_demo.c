...
//
static void evTick(state_t* state) {
    ENTER;
    furi_assert(state);
    ...
    if(animate(state)) { // true if edge of screen tapped
        ...
        if(furi_hal_speaker_is_mine() || furi_hal_speaker_acquire(25)) {
            ...
            furi_hal_speaker_start(
                penta[rand() % 5], 0.5); // start playing a random note (from the list)
        }
...
//
static inline bool evKey(eventMsg_t* msg, state_t* state, Gui* gui) {
    ENTER;
    furi_assert(msg);
    furi_assert(state);
    furi_assert(gui);
...
//
static bool move(state_t* state, dir_t dir) {
    ENTER;
    furi_assert(state);
...
//
static void animateSet(animID_t id, state_t* const state, const Gui* gui) {
    ENTER;
    furi_assert(state);
    furi_assert(gui);
...
//
static void animateEn(state_t* state, bool on) {
    ENTER;
    furi_assert(state);
...
//
static int animate(state_t* state) {
    ENTER;
    furi_assert(state);
...
//
static inline bool stateInit(state_t* const state, const Gui* gui) {
    ENTER;
    furi_assert(state);
    furi_assert(gui);
