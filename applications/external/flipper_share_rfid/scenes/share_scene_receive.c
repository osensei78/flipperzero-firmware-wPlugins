...
#define FSH_IDLE_OPERATION 50 //ms

#define TAG "ShareRecv"
...
static int32_t file_read_worker_thread(void* context) {
    ...
    fsh_init_from_external_receive();

    bool field_stopped = false;

    while(is_running) {
        fsh_idle();
        furi_delay_ms(FSH_IDLE_OPERATION);
        ...
        if(finished) {
            ...
            // RFID carrier while the result screen waits for the user. Only sets a
            // flag; the reader is fully stopped/freed in on_exit on the scene
            // thread. Harmless if it kept polling (section 7), but idling the bus
            // is cleaner.
            if(!field_stopped) {
                rfid_transport_stop_field();
                field_stopped = true;
            }
        }
...
static void progress_view_draw_callback(Canvas* canvas, void* context) {
    ...
    // fsh_parts concurrently). Skip on contention rather than block the renderer.
    char fname[FSH_FILENAME_LENGTH];
    uint32_t fsize = 0;
    uint32_t rcv = 0, need = 0, start = 0, last_progress = 0;
    bool finalizing = false;
    uint8_t levels[FSH_PARTS_COUNT];
    if(fsh_try_lock_ms(10)) {
    ...
    elements_multiline_text_aligned(
        ...
        AlignCenter,
        AlignTop,
        "Receiving via " FSH_TRANSPORT_NAME "...");
    ...
    // If no new block has arrived for FSH_STALL_MS, show "stalled" instead of a
    ...
    for(uint32_t i = 0; i < FSH_PARTS_COUNT; ++i) {
        uint8_t lv = levels[i];
        if(!lv) continue;
        int fill = (lv * h) / 255;
        if(fill < 1) fill = 1;
...
static void progress_view_init(ShareApp* app) {
    ...
    view_dispatcher_add_view(app->view_dispatcher, ShareViewIdProgress, progress_view);
}

static void progress_view_deinit(ShareApp* app) {
    if(!progress_view) return;
    view_dispatcher_remove_view(app->view_dispatcher, ShareViewIdProgress);
    view_free(progress_view);
    progress_view = NULL;
    progress_view_active = false;
}

void share_scene_receive_on_enter(void* context) {
    ShareApp* app = context;

    // Create the shared-state lock BEFORE starting the worker thread and the
    // RFID transport, so both threads see a valid mutex from their first tick.
    fsh_lock_ensure();
    ...
    // Receiver role: reader; drives the 125 kHz carrier and demodulates.
    rfid_transport_init(RfidTransportModeReader);
}
...
static void update_timer_callback(void* context) {
    ...
    if(!fsh_try_lock_ms(20)) return; // skip this tick on contention
    if(complete) {
        is_success = g.r_is_success;
        // Actual receive throughput + elapsed time (r_start_ms..r_finfsh_ms).
        uint32_t st = g.r_start_ms, fin = g.r_finfsh_ms, fsz = g.r_file_size;
        uint32_t el_ms = (fin > st) ? (fin - st) : 0;
        uint32_t bps = (el_ms > 0) ? (uint32_t)((uint64_t)fsz * 1000u / el_ms) : 0;
        char tbuf[16];
        fsh_fmt_duration(el_ms / 1000u, tbuf, sizeof(tbuf));
        snprintf(
            progress_text,
    ...
    if(complete) {
        ...
        if(!progress_view_active) {
            view_dispatcher_switch_to_view(app->view_dispatcher, ShareViewIdProgress);
        ...
        snprintf(progress_text, sizeof(progress_text), "Hold ~2cm apart\nWaiting for announce...");
...
bool share_scene_receive_on_event(void* context, SceneManagerEvent event) {
    ShareApp* app = context;
    bool consumed = false;
...
void share_scene_receive_on_exit(void* context) {
    ...
    // Worker thread is joined in on_event and the RFID transport is stopped above,
