...
void share_scene_show_file_on_enter(void* context) {
    if(!context) {
        return;
    }
...
bool share_scene_show_file_on_event(void* context, SceneManagerEvent event) {
    if(!context) {
        return false;
    }
    ...
    if(event.type == SceneManagerEventTypeCustom) {
        if(event.event == DialogExResultLeft) {
            ...
            if(app->scene_manager) {
                scene_manager_next_scene(app->scene_manager, ShareSceneSend);
            }
            consumed = true;
        }
...
void share_scene_show_file_on_exit(void* context) {
    UNUSED(context);
}
