...
static int32_t file_read_worker_thread(void* context) {
    ...
    if(!fsh_init_from_external_transmit(app->selected_file_path)) {
...
void share_scene_send_on_enter(void* context) {
    ShareApp* app = context;
    ...
    // Sender role: load-modulating tag; the receiver drives the 125 kHz field.
    rfid_transport_init(RfidTransportModeTag);
}
...
static void update_timer_callback(void* context) {
    ...
    if(state->reading_complete) {
        ...
        // While fsh_init hashes the file (chunked MD5) show its progress; the
        // regular name/size/ETA text takes over once the hash completes.
        uint32_t hash_done, hash_total;
        if(!fsh_hash_progress_get(&hash_done, &hash_total)) return;
...
bool share_scene_send_on_event(void* context, SceneManagerEvent event) {
    ShareApp* app = context;
    bool consumed = false;
