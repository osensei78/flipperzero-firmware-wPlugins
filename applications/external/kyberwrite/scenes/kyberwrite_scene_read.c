...
static void read_callback(LFRFIDWorkerReadResult result, ProtocolId protocol, void* context) {
    ...
    if(result == LFRFIDWorkerReadDone) {
        scene_manager_set_scene_state(app->scene_manager, KyberSceneRead, (uint32_t)protocol);
...
static void show_result(KyberApp* app, ProtocolId protocol) {
    ...
    if(protocol != LFRFIDProtocolEM4100 && protocol != LFRFIDProtocolEM410032 &&
       protocol != LFRFIDProtocolEM410016) {
        widget_add_string_element(
...
bool kyberwrite_scene_read_on_event(void* context, SceneManagerEvent event) {
    ...
    if(event.type == SceneManagerEventTypeCustom) {
        if(event.event == KyberEventReadDone) {
            ...
            ProtocolId protocol =
                (ProtocolId)scene_manager_get_scene_state(app->scene_manager, KyberSceneRead);
