...
void calculate_values(void* context) {
    ...
    free(ptr1);
    free(ptr2);
    free(ptr3); // TODO I don't think this is strictly necessary
...
void voltcalc_scenes_menu_callback(void* context, uint32_t index) {
    ...
    case VoltcalcScenesMainMenuSceneVoltage:
        scene_manager_handle_custom_event(
            app->scene_manager, VoltcalcScenesMainMenuSceneVoltageEvent);
        break;
    case VoltcalcScenesMainMenuSceneResistance:
        scene_manager_handle_custom_event(
            app->scene_manager, VoltcalcScenesMainMenuSceneResistanceEvent);
        break;
    case VoltcalcScenesMainMenuSceneCurrent:
        scene_manager_handle_custom_event(
            app->scene_manager, VoltcalcScenesMainMenuSceneCurrentEvent);
        break;
    case VoltcalcScenesMainMenuSceneCalculate:
        scene_manager_handle_custom_event(
            app->scene_manager, VoltcalcScenesMainMenuSceneCalculateEvent);
        break;
    case VoltcalcScenesMainMenuSceneAbout:
        scene_manager_handle_custom_event(
            app->scene_manager, VoltcalcScenesMainMenuSceneAboutEvent);
        break;
    }
}
...
void voltcalc_scenes_voltage_callback(void* context) {
    App* app = context;
    scene_manager_handle_custom_event(app->scene_manager, VoltcalcScenesVoltageSceneSaveEvent);
}

void voltcalc_scenes_resistance_callback(void* context) {
    App* app = context;
    scene_manager_handle_custom_event(app->scene_manager, VoltcalcScenesResistanceSceneSaveEvent);
}

void voltcalc_scenes_current_callback(void* context) {
    App* app = context;
    scene_manager_handle_custom_event(app->scene_manager, VoltcalcScenesCurrentSceneSaveEvent);
}
...
void voltcalc_scenes_calculate_scene_on_enter(void* context) {
    ...
    widget_add_string_element(
        app->widget, 5, 5, AlignLeft, AlignCenter, FontPrimary, furi_string_get_cstr(message));
    ...
    widget_add_string_element(
        app->widget, 5, 25, AlignLeft, AlignCenter, FontPrimary, furi_string_get_cstr(message));
...
static App* app_alloc() {
    ...
    app->scene_manager = scene_manager_alloc(&voltcalc_scenes_scene_manager_handlers, app);
    app->view_dispatcher = view_dispatcher_alloc();
    view_dispatcher_set_event_callback_context(app->view_dispatcher, app);
    view_dispatcher_set_custom_event_callback(
        app->view_dispatcher, voltcalc_scene_custom_callback);
    view_dispatcher_set_navigation_event_callback(
        app->view_dispatcher, voltcalc_scene_back_event_callback);
    app->submenu = submenu_alloc();
    view_dispatcher_add_view(
        app->view_dispatcher, VoltcalcScenesSubmenuView, submenu_get_view(app->submenu));
    app->widget = widget_alloc();
    view_dispatcher_add_view(
        app->view_dispatcher, VoltcalcScenesWidgetView, widget_get_view(app->widget));
    app->text_input = text_input_alloc();
    view_dispatcher_add_view(
        app->view_dispatcher, VoltcalcScenesTextInputView, text_input_get_view(app->text_input));
    return app;
}
...
int32_t voltcalc_app(void* p) {
    ...
    scene_manager_next_scene(app->scene_manager, VoltcalcScenesMainMenuScene);
