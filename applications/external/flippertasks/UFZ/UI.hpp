...
public:
    ...
    const ButtonPanel&
        addLabel(uint16_t x, uint16_t y, Font font, const char* label) const noexcept;
...
public:
    ...
    void setResultCallback(
        ByteInputCallback inputCallback,
        ByteChangedCallback changedCallback,
        void* context,
        uint8_t* bytes,
        uint8_t bytesCount) const noexcept;
...
public:
    ...
    void setResultCallback(
        NumberInputCallback inputCallback,
        void* context,
...
public:
    ...
    const DialogEx&
        setHeader(const char* text, uint8_t x, uint8_t y, Align horizontal, Align vertical)
            const noexcept;
    const DialogEx&
        setText(const char* text, uint8_t x, uint8_t y, Align horizontal, Align vertical)
            const noexcept;
...
public:
    const Widget& addStringMultilineElement(
        uint8_t x,
        uint8_t y,
        Align horizontal,
        Align vertical,
    ...
    const Widget& addStringElement(
        ...
        Font font,
        const char* text) const noexcept;
    const Widget& addTextBoxElement(
        uint8_t x,
        uint8_t y,
        uint8_t width,
        uint8_t height,
        ...
        Align vertical,
        const char* text,
        bool stripToDots) const noexcept;
    const Widget&
        addTextScrollElement(uint8_t x, uint8_t y, uint8_t width, uint8_t height, const char* text)
            const noexcept;
    const Widget& addButtonElement(
        GuiButtonType type,
        const char* text,
